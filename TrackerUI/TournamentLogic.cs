    ...
    public static class TournamentLogic
    {
        ...
        public static void AlterUsersToNewRound(this TournamentModel model)
        {
            ...
            foreach (MatchupModel matchup in currentRound)
            {
                foreach (MatchupEntryModel me in matchup.Entries)
                {
                    ...
                    {
                        AlterPersonToNewRound(p, me.TeamCompeting.TeamName, matchup.Entries.Where(x => x.TeamCompeting != me.TeamCompeting).FirstOrDefault());
                    }
                }
            }
        }
        ...
        private static void CompleteTournament(TournamentModel model)
        {
            ...
            if (model.Prizes.Count>0)
            {
                ...
                {
                    runnerUpPriza = secondPlacePrize.CalculatePrizePayout(totalIncome);
                }
            }
            string subject = "";
            StringBuilder body = new StringBuilder();
            ...
            if (runnerUpPriza>0)
            {
                body.AppendLine($"<p>{ runnerUp.TeamName } will receive ${ runnerUpPriza }</p>");
            }
